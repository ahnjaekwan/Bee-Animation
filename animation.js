...
Animation.prototype.display = function(time)
	{
		...
		var purplePlastic = new Material(vec4(.9, .5, .9, 1), .2, .8, .5, 40), // Omit the final (string) parameter if you want no texture
			greyPlastic = new Material(vec4(.5, .5, .5, 1), .8, 1, 1, 40),
			redPlastic = new Material(vec4(255, 0, 0), .2, .8, .5, 20),
            yellowPlastic = new Material(vec4(255, 255, 0), .2, .8, .5, 20),
            brownPlastic = new Material(vec4(165, 42, 42), .2, .8, .5, 20),
            bluePlastic = new Material(vec4(0, 0, 255), .8, .8, .5, 20),
            blackPlastic = new Material(vec4(0, 0, 0), .2, .8, .5, 20),
    ...
    //trunk
	    ...
	    {
            if(i!=0)
	            model_transform = mult(model_transform, rotation(3 * Math.sin(this.graphicsState.animation_time / 3000), 0, 0, 1));
	        model_transform = mult(model_transform, scale(0.4, 2, 0.4));
	        model_transform = mult(model_transform, translation(0, 0.5, 0));
	        this.m_cube.draw(this.graphicsState, model_transform, brownPlastic);
	        model_transform = mult(model_transform, scale(1/0.4, 0.5, 1/0.4));
	    }
    //foliage
        model_transform = stack.pop();
        stack.push(model_transform);
        model_transform = mult(model_transform, rotation(15 * Math.sin(this.graphicsState.animation_time / 3000), 0, 0, 1));
    ...
    //chest
        ...
        model_transform = mult(model_transform, rotation(50, 0, 1, 0));
        model_transform = mult(model_transform, translation(0, 7, -10));
        model_transform = mult(model_transform, translation(0, Math.sin(this.graphicsState.animation_time / 2000), 0));
        stack.push(model_transform);
    ...
    //wing
        for (var i = -1; i < 2; i += 2)
        {
            model_transform = stack.pop();
            stack.push(model_transform);
            model_transform = mult(model_transform, translation(0, 0.5, 0.5 * i));
            model_transform = mult(model_transform, rotation(i * 50 * Math.sin(this.graphicsState.animation_time / 2000), 1, 0, 0));
            model_transform = mult(model_transform, translation(0, 0, 1.5 * i));
    ...
    //leg
        for (var i = -1; i < 2; i += 2)
        {
            model_transform = stack.pop();
            stack.push(model_transform);
            model_transform = mult(model_transform, rotation( i * 10 * Math.sin(this.graphicsState.animation_time / 2000 ), 1, 0, 0));
            model_transform = mult(model_transform, translation(-0.5, -0.9, 0.5*i));
            model_transform = mult(model_transform, scale(0.3, 1, 0.3));
            for (var j = 0; j < 2; j++)
            {
                model_transform = mult(model_transform, rotation( i * 10 + i * 10 * Math.sin(this.graphicsState.animation_time / 2000), 1, 0, 0));
                model_transform = mult(model_transform, translation(0, -1*j, 0));
                this.m_cube.draw(this.graphicsState, model_transform, greyPlastic);
                model_transform = mult(model_transform, translation(2+j*(-4), 0, 0));
                this.m_cube.draw(this.graphicsState, model_transform, greyPlastic);
                model_transform = mult(model_transform, translation(2+j*(-4), 0, 0));
                this.m_cube.draw(this.graphicsState, model_transform, greyPlastic);
            }
